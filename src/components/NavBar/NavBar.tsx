      ...
      <div className="flex flex-col w-full md:items-center md:justify-between md:flex-row">
        ...
        <nav className={`${navHidden} ${burgerOpen} flex flex-grow pb-4 xs:pb-2 justify-center items-center md:pb-0`}>
          {header.allDatoCmsMenu.edges.map(({ node: menuItem }, key: number) => (
            <div
              key={`menuItem_${key}`}
              className="px-4 py-2 text-sm text-gray-900 rounded-lg md:mt-0 focus:outline-none focus:shadow-outline hover:text-helens-blue transition-all duration-300 uppercase font-thin xs:p-3 xs:text-xs xs:px-2 md:p-4 md:text-base"
            >
          ...
          {!bottomNav && (
            <Link to="/contact">
              <button className="border border-helens-blue bg-helens-blue hover:bg-gray-900 transition-all duration-500 text-white inline-block rounded-sm px-4 py-2 text-center uppercase items-center ml-2">
                ...
                <span className="font-thin xs:text-xs md:text-base">Book Now</span>
              </button>
            </Link>
          )}
        </nav>
