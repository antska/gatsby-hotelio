...
const NavBar = ({ bottomNav = false, withLogo = false }: Props) => {
  ...
  return (
    <div className={`nav ${bottomNav ? 'bottom-nav' : 'top-nav'}`}>
      {withLogo && !bottomNav && (
        <Link to="/" className="inline-block w-32 h-auto align-middle ml-8">
      ...
      <div className="mr-8">
        {header.allDatoCmsMenu.edges.map(({ node: menuItem }, key: number) => (
          <div
            key={`menuItem_${key}`}
            className="inline-block p-4 hover:text-helens-blue transition-all duration-300 uppercase font-thin"
          >
